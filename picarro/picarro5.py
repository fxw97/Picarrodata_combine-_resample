...
data_mean = data.groupby('day_number')['CH4_dry'].mean()
data_std = data.groupby('day_number')['CH4_dry'].std()

data_upper = data_mean + 3*data_std
data_lower = data_mean - 3*data_std

# 为每行数据填充上限值与下限值
data_upper_list = [val for val in data_upper for i in range(288)]
data_lower_list = [val for val in data_lower for i in range(288)]
...
data.to_excel('finally_222.xlsx')